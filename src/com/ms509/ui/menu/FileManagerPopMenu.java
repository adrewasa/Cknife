        ...
        public void actionPerformed(final ActionEvent e) {
            ...
            switch (e.getActionCommand()) {
                ...
                case "文件夹":
                    ...
                    model.setEdit(false);
                    break;
                case CommonConstant.TEXT.DOWNLOAD:
                    String name = list.getValueAt(list.getSelectedRow(), 1)
                            .toString();
                    abpath = path.getText() + name;

                    final JFileChooser downch = new JFileChooser( new Configuration().getValue(CommonConstant.CONF.DOWNLOAD_BASE_DIR));
                    downch.setDialogTitle("下载文件到本地");
                    File df = new File(name);
                    downch.setSelectedFile(df);
                    ...
                    if (select == JFileChooser.APPROVE_OPTION) {
                        try {
                            ...
                            Runnable dlrun = new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    if (bytes != null) {
                                        try {
                                            ...
                                            saveLastDownloadDir(f.getParent());
                                ...
                                private void saveLastDownloadDir(String ldir) {

                                    lastDownloadDir = ldir;
                                }

                                private String getLastDownloadDir() {

                                    File f = new File(lastDownloadDir);
                                    if (!f.exists() || !f.isDirectory()) {
                                        return ".";
                                    }
                                    return lastDownloadDir;
                                }
                            };
